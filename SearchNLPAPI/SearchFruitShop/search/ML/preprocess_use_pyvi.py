...
def preprocess_data_form_dict(dict_data):
    dict_preprocessed_products = {}
    # Lưu vào dictionary mới với id sản phẩm là key và preprocessed detail là value
    for data_id, data_detail in dict_data.items():
        preprocessed_detail = custom_preprocessing(data_detail)
        dict_preprocessed_products[data_id] = preprocessed_detail
    return dict_preprocessed_products
